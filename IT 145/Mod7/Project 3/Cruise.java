...
public class Cruise {
    ...
    public void printCruiseDetails() {
    	...
    	int spaceCount;
        String spaces = "";
        spaceCount = 18 - cruiseName.length();

        for (int i = 1; i <= spaceCount; i++) {
            spaces = spaces + " ";
        }
    	
        // created below coding to implement cruise's detail under specific categories for each information.
    	System.out.println(cruiseName + "\t" + spaces + cruiseShipName + "\t\t" + departurePort + "\t\t" + spaces + destination
    						+ "\t\t" + returnPort);
